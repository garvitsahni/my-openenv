...
def _http_request(method, path, payload=None, timeout=30.0):
    body = None
    headers = {}
    if payload is not None:
        body = json.dumps(payload).encode("utf-8")
...
def call_with_retry(model: str, prompt: str, env_type: str, max_retries: int = 3) -> dict:
    ...
    # Try LiteLLM with API key from environment
...
def run_task(task_id: str, env_type: str) -> None:
    ...
    if status_code != 200 or not isinstance(data, dict):
        print(f"Failed to reset: {text}", flush=True)
        emit_block("[END]", {
            "task": task_id, "env": env_type,
            "score": 0.001, "steps": 0, "status": "reset_failed"
        })
        return
    ...
    try:
        while not done:
            ...
            try:
                ...
                if step_status != 200 or not isinstance(sdata, dict):
                    print(f"FAILED STEP: {step_text}", flush=True)
                    break

                obs = sdata.get("observation", {})
                reward = float(sdata.get("reward", 0.0))
                done = bool(sdata.get("done", True))
                cum_score += reward
                ...
                emit_block("[STEP]", {
                    ...
                    "action": action.get("action_type", "unknown"),
                    "reward": reward,
                    "score": min(max(cum_score, 0.001), 0.999),
                    "done": done,
                })

            except Exception as e:
                print(f"Error during step: {e}", flush=True)
                done = True
    ...
    try:
        score_status, _, fsc = _http_request("GET", f"/score/{episode_id}", None)
        if score_status == 200 and isinstance(fsc, dict) and "final_score" in fsc:
            final_score = float(fsc["final_score"])
    except Exception:
        pass

    safe_score = min(max(final_score, 0.001), 0.999)
    print(f"Task {task_id} Final Score: {safe_score:.3f}", flush=True)
    emit_block("[END]", {
        "task": task_id, "env": env_type,
        "score": safe_score, "steps": step, "status": "ok"
    })
...
if __name__ == "__main__":
    ...
    for t_id, t_env in TASKS:
        try:
            ...
            emit_block("[END]", {
                "task": t_id, "env": t_env,
                "score": 0.001, "steps": 0, "status": "error"
            })
        time.sleep(2)
>>>>>>> c6dd658 (fix: restore inference script with LiteLLM and strict score clamping)
